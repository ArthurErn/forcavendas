  ...
  Future post(context) async {
    ...
    for (int i = 0; i < headerData.length; i++) {
      ...
      var response = await HttpPost().connect(
          'http://${ipController.text}/lotuserp/fvenpost12_vendas',
          concatJson,
          context);
      //print(concatJson);
        _db.rawUpdate(
            '''UPDATE vendas SET situacao = 1, id_venda = $response WHERE id_venda = ${headerData[i]['id_venda']};''');
        _db.rawUpdate(
            '''UPDATE vendas_itens SET enviado = 1, id_venda = $response WHERE id_venda = ${headerData[i]['id_venda']};''');
    }
  }
}
